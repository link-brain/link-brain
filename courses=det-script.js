document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        // showToast('خطأ في تحميل مكتبة الأمان (DOMPurify)، يرجى إعادة تحميل الصفحة', 'error'); // لا يمكن استدعاء showToast هنا
    ...
    function setupTooltips() {
        ...
        // Popper.js يتم تحميله كـ <script> عادي في HTML، وليس كوحدة (module)
    ...
    function openFeaturesPopup(event) {
        ...
        if (featuresText) {
            ...
            featuresArray.forEach(feature => {
                // استخدام DOMPurify بعد التأكد من وجوده
    ...
    try {
        ...
        function handleAuthStateChanged(user) {
            ...
            try {
                if (user) {
                    ...
                    initializeRatings(); // لا تحتاج db/auth هنا لأنهما عامان
                } else {
                    elements.googleLoginBtn.innerHTML = `
        ...
        async function sendMessage() {
            console.log('sendMessage called. auth.currentUser is:', auth.currentUser); // استخدام auth
            if (!firebaseInitialized) {
                showToast('خطأ في تهيئة Firebase، يرجى إعادة تحميل الصفحة', 'error');
                return;
            }
            ...
            try {
                ...
                await addDoc(collection(db, 'messages'), { // استخدام db
        ...
        async function loadMessages() {
            ...
            let messagesQuery = query(
                collection(db, 'messages'), // استخدام db
            ...
            try {
                unsubscribeMessages = onSnapshot(messagesQuery, (snapshot) => {
                    ...
                    messages.forEach((msg) => {
                        ...
                        messageElement.className = `message ${msg.userId === auth.currentUser?.uid ? 'user-message' : ''}`; // استخدام auth
        ...
        async function handleRating(event) {
            ...
            console.log('handleRating called. auth.currentUser is:', auth.currentUser); // استخدام auth
            if (!firebaseInitialized) {
                showToast('خطأ في تهيئة Firebase، يرجى إعادة تحميل الصفحة', 'error');
                return;
            }
            const user = auth.currentUser; // استخدام auth
            if (!user) {
            ...
            try {
                ...
                const ratingRef = doc(db, 'ratings', `${linkId}_${user.uid}`); // استخدام db
                ...
                await updateRatingUI(linkId); // لا تحتاج db/auth هنا
            } catch (error) {
        ...
        async function updateRatingUI(linkId) {
            ...
            try {
                const ratingsQuery = query(collection(db, 'ratings'), where('linkId', '==', linkId)); // استخدام db
            ...
            document.querySelectorAll('.resource-card').forEach(card => {
                ...
                if (!unsubscribeRatings[linkId]) {
                    unsubscribeRatings[linkId] = onSnapshot(
                        query(collection(db, 'ratings'), where('linkId', '==', linkId)), // استخدام db
        ...
        function setupFirebaseEventListeners() {
            ...
            onAuthStateChanged(auth, handleAuthStateChanged); // استخدام auth

            // تهيئة التقييمات عند تحميل الصفحة إذا كان المستخدم مسجل الدخول بالفعل
            if (auth.currentUser) { // استخدام auth
                initializeRatings();
            }
        }
